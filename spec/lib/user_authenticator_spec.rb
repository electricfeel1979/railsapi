...
describe UserAuthenticator do
    let(:user) { create :user, login: 'kevimm', password: 'password' }
    ...
    context 'when initialized with login & password' do
        let(:authenticator) { described_class.new(login: 'kevinmm', password: 'secret')}
        let(:authenticator_class) {UserAuthenticator::Standard}

        describe '#initialize' do
            it 'should initialize proper authenticator' do
                expect(authenticator_class).to receive(:new).with('kevinmm', 'secret')
                authenticator
            end
        end
